...
CREATE TABLE IF NOT EXISTS `BanderaAzul`.`cambioClave` (
  ...
  `claveNueva` VARCHAR(45) NOT NULL,
  `claveVieja` VARCHAR(45) NOT NULL,
  `Persona_cedula` INT NOT NULL,
  `fecCreacion` DATETIME NOT NULL,
  `usuarioCreacion` VARCHAR(45) NOT NULL,
  `fecUltimaModificacion` DATETIME NULL,
  ...
  INDEX `fk_cambioClave_Persona1_idx` (`Persona_cedula` ASC),
  CONSTRAINT `fk_cambioClave_Persona1`
    FOREIGN KEY (`Persona_cedula`)
    REFERENCES `BanderaAzul`.`Persona` (`cedula`)
    ON DELETE NO ACTION
    ON UPDATE NO ACTION)
...
DELIMITER $$
USE `BanderaAzul`$$
CREATE PROCEDURE `obtenerCanton` ()
	NOT DETERMINISTIC 
    READS SQL DATA 
    SQL SECURITY DEFINER
BEGIN
...
DELIMITER $$
USE `BanderaAzul`$$
CREATE PROCEDURE `obtenerDistrito` ()
	NOT DETERMINISTIC 
    READS SQL DATA 
    SQL SECURITY DEFINER
BEGIN
...
DELIMITER $$
USE `BanderaAzul`$$
CREATE PROCEDURE `insertarUsuario`(IN `pCedula` INT, IN `pNombre` VARCHAR(45), IN `pApellido1` VARCHAR(45), IN `pApellido2` VARCHAR(45), IN `pPais` INT, IN `pUser` VARCHAR(45), IN `pPass` VARCHAR(45), IN `pTipo` INT)
	NOT DETERMINISTIC 
    CONTAINS SQL 
    SQL SECURITY DEFINER
BEGIN
	 INSERT INTO `persona`(`cedula`, `nombrePersona`, `primerApellidoPesona`, `segundoApellidoPersona`, `Pais_idPais`) VALUES (pCedula, pNombre, pApellido1, pApellido2, pPais); 
...
-- -----------------------------------------------------
-- procedure obtenerCentrosAcopio
-- -----------------------------------------------------

USE `BanderaAzul`;
DROP procedure IF EXISTS `BanderaAzul`.`obtenerCentrosAcopio`;

DELIMITER $$
USE `BanderaAzul`$$
CREATE PROCEDURE `obtenerCentrosAcopio` ()
	NOT DETERMINISTIC 
    READS SQL DATA 
    SQL SECURITY DEFINER 
BEGIN 
