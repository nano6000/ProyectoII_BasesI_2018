...
CREATE TABLE IF NOT EXISTS `BanderaAzul`.`EquivalenciaMaterial` (
  ...
  `nombre` VARCHAR(45) NOT NULL COMMENT 'Nombre de la equivalencia del material ',
...
CREATE TABLE IF NOT EXISTS `BanderaAzul`.`Puntos` (
  ...
  `cantidad` VARCHAR(45) NOT NULL COMMENT 'Cantidad de puntos',
  `fechaHoraTransaccionPuntos` DATETIME NOT NULL COMMENT 'Fecha y hora de la transacción de los puntos',
  `Usuario_nombreUsuario` VARCHAR(45) NOT NULL COMMENT 'Llave foránea',
...
DELIMITER $$
USE `BanderaAzul`$$
CREATE PROCEDURE `insertarCanje`(IN `pProducto` VARCHAR(45), IN `pUser` VARCHAR(45)) 
	NOT DETERMINISTIC 
    CONTAINS SQL 
    SQL SECURITY DEFINER 
BEGIN 
	INSERT INTO `productoxusuario`(`Producto_idProducto`, `Usuario_nombreUsuario`) 
		VALUES ((SELECT idProducto FROM `producto` WHERE producto.nombre = pProducto), pUser);
	UPDATE `Usuario`
        SET `Usuario`.`puntosActuales`  = `Usuario`.`puntosActuales` - (SELECT `producto`.`precio` FROM `producto` WHERE `producto`.`nombre` = pProducto),
			`Usuario`.`puntosGastados`  = `Usuario`.`puntosGastados` + (SELECT `producto`.`precio` FROM `producto` WHERE `producto`.`nombre` = pProducto)
...
DELIMITER $$
USE `BanderaAzul`$$
CREATE PROCEDURE `usuariosXedad`() NOT DETERMINISTIC READS SQL DATA SQL SECURITY DEFINER BEGIN
    SELECT
