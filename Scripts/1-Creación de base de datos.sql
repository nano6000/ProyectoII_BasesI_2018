...
CREATE PROCEDURE `top5Productos`(IN `pComercio` INT) 
    ...
    SQL SECURITY DEFINER
BEGIN
	SELECT * FROM (SELECT p.Comercio_idComercio as comercio, p.nombre, COUNT(pxu.idCodigo)
		FROM
			producto p,
			productoxusuario pxu
		WHERE
			p.idProducto = pxu.Producto_idProducto
		GROUP BY p.idProducto
		ORDER BY COUNT(pxu.idCodigo) DESC
		LIMIT 5) AS t
		WHERE t.comercio = pComercio;
END$$
