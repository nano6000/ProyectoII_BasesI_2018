...
DELIMITER $$
USE `BanderaAzul`$$
CREATE  PROCEDURE `insertarCanje`(IN `pProducto` INT, IN `pUser` VARCHAR(45), IN `pPrecio` INT) 
	NOT DETERMINISTIC 
    CONTAINS SQL 
    SQL SECURITY DEFINER 
BEGIN 
	...
	UPDATE `Usuario` SET `Usuario`.`puntosActuales` = `Usuario`.`puntosActuales` - pPrecio, 
						 `Usuario`.`puntosGastados` = `Usuario`.`puntosGastados` + pPrecio 
...
-- -----------------------------------------------------
-- procedure obtenerPrecioProducto
-- -----------------------------------------------------

USE `BanderaAzul`;
DROP procedure IF EXISTS `BanderaAzul`.`obtenerPrecioProducto`;

DELIMITER $$
USE `BanderaAzul`$$
CREATE PROCEDURE `obtenerPrecioProducto`(IN `pId` INT) 
	NOT DETERMINISTIC 
    READS SQL DATA 
    SQL SECURITY DEFINER 
BEGIN
	SELECT precio FROM producto WHERE producto.idProducto = pId;
END$$
