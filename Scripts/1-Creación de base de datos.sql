...
BEGIN 
	INSERT INTO `productoxusuario`(`Producto_idProducto`, `Usuario_nombreUsuario`) 
		VALUES (pProducto, pUser);
	UPDATE `Usuario`
        SET `Usuario`.`puntosActuales`  = `Usuario`.`puntosActuales` - (SELECT `producto`.`precio` FROM `producto`, `productoxusuario` pxu WHERE `producto`.`nombre` = pProducto AND LAST_INSERT_ID() = pxu.idCodigo),
			`Usuario`.`puntosGastados`  = `Usuario`.`puntosGastados` + (SELECT `producto`.`precio` FROM `producto`, `productoxusuario` pxu WHERE `producto`.`nombre` = pProducto AND LAST_INSERT_ID() = pxu.idCodigo)
