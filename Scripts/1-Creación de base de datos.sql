...
-- -----------------------------------------------------
-- procedure obtenerPais
-- -----------------------------------------------------

USE `BanderaAzul`;
DROP procedure IF EXISTS `BanderaAzul`.`obtenerPais`;
...
BEGIN 
    ...
    FROM `Canton` 
    WHERE Provincia_idProvina = pCodigo; 
END$$
...
DELIMITER $$
USE `BanderaAzul`$$
CREATE FUNCTION `puedeCanjear`(`totalPrecio` INT, `pUser` VARCHAR(45), `resp` BOOLEAN) RETURNS TINYINT(1) 
	NOT DETERMINISTIC 
    READS SQL DATA 
    SQL SECURITY DEFINER 
BEGIN 
	SELECT IF(totalPrecio<=(select puntosActuales from `Usuario` where nombreUsuario = pUser), TRUE, FALSE) INTO resp; 
	RETURN resp; 
END$$
